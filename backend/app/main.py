...
class SQLiteScanStore:
    def __init__(self, db_file="scan_results.db", ttl=2592000):
...
async def get_demo_apps():
    ...
    demo_apps_path = os.path.join(base_path, "demo-apps")
    demo_apps = [
        {
            "id": "python-flask",
            "name": "Python Flask",
            "description": "Vulnerable Python web application",
            "icon": "🐍",
            "path": os.path.join(demo_apps_path, "python-flask"),
            "language": "Python",
        ...
        },
        {
            ...
            "description": "Vulnerable Node.js API service",
            "icon": "📦",
            "path": os.path.join(demo_apps_path, "node-express"),
            "language": "JavaScript",
            "vulnerabilities": ["Prototype Pollution", "Path Traversal", "Insecure Dependencies"]
        },
        {
            "id": "java-spring",
            "name": "Java Spring",
            "description": "Vulnerable Java enterprise app",
            "icon": "☕",
            "path": os.path.join(demo_apps_path, "java-spring"),
            "language": "Java",
            "vulnerabilities": ["Deserialization", "XXE", "SSRF"]
        },
        {
            "id": "go-gin",
            "name": "Go Gin",
            "description": "Vulnerable Go microservice",
            "icon": "⚡",
            "path": os.path.join(demo_apps_path, "go-gin"),
            "language": "Go",
            "vulnerabilities": ["Race Conditions", "Improper Error Handling"]
        }
    ]
    available_apps = []
    for app in demo_apps:
        if os.path.exists(app["path"]):
            ...
            logger.warning(f"Demo app not found: {app['path']}")
    return {
        "demo_apps": available_apps,
...
async def run_scan(scan_id: str, request: ScanRequest):
    try:
        ...
        if "dast" in request.scan_types or "all" in request.scan_types:
            ...
            if request.target_url:
                try:
                    ...
                    zap_findings = await asyncio.wait_for(zap_task, timeout=1800.0)
        ...
        threats = []
        attack_graph = {}
        if "threat_model" in request.scan_types or "threat_modeling" in request.scan_types or "all" in request.scan_types:
...
async def generate_report(scan_id: str, format: str = "txt"):
    if scan_id not in scan_results_store:
    ...
    if format.lower() == "txt":
        ...
        report_lines.append("THREATMODELX SECURITY SCAN REPORT".center(80))
        report_lines.append("="*80)
        report_lines.append("")
        report_lines.append(f"Scan ID: {scan_id}")
        report_lines.append(f"Timestamp: {result.get('timestamp', datetime.utcnow().isoformat())}")
        report_lines.append(f"Repository: {result.get('repo_path', 'N/A')}")
        report_lines.append("")
        report_lines.append("-"*80)
        report_lines.append("SUMMARY")
        report_lines.append("-"*80)
        report_lines.append(f"Total Findings: {len(findings)}")
        report_lines.append(f"Total Threats: {len(threats)}")
        report_lines.append(f"Risk Score: {result.get('risk_score', 0):.2f}")
        report_lines.append("")
        severity_breakdown = {
        ...
        if findings:
            ...
            for idx, finding in enumerate(findings, 1):
                report_lines.append(f"[{idx}] {finding.get('severity', 'UNKNOWN')} - {finding.get('description', 'No description')}")
                report_lines.append(f"    Tool: {finding.get('tool', 'Unknown')}")
                report_lines.append(f"    File: {finding.get('file', 'Unknown')}:{finding.get('line', 'N/A')}")
                if finding.get('cwe'):
                    report_lines.append(f"    CWE: {finding.get('cwe')}")
                if finding.get('fix_suggestion'):
                    report_lines.append(f"    Fix: {finding.get('fix_suggestion')}")
                report_lines.append("")
        if threats:
            report_lines.append("="*80)
            ...
            for idx, threat in enumerate(threats, 1):
                report_lines.append(f"[{idx}] {threat.get('category', 'UNKNOWN')} - {threat.get('description', 'No description')}")
                report_lines.append(f"    Risk Level: {threat.get('risk_level', 'Unknown')}")
                report_lines.append(f"    Component: {threat.get('component', 'Unknown')}")
                if threat.get('mitigation'):
                    report_lines.append(f"    Mitigation: {threat.get('mitigation')}")
                report_lines.append("")
        report_lines.append("="*80)
        ...
        return PlainTextResponse(
            ...
            headers={
                "Content-Disposition": f"attachment; filename=security_report_{scan_id}.txt"
            }
        )
    else:
